...
import Router, { BrowserRouter, Link, Route, Routes, useNavigate } from 'react-router-dom'
...
function MainPage() {
    const [selected, setSelected] = useState({year:'', branch:'', heldin: ''})
    ...
    const HandlerSubmit = () =>{ 
        ...
        if(year.current.value && branch.current.value && heldin.current.value){
            axios.post('http://localhost:8085/api/check',
            {
    ...
    <main>           
            <div className="notiDiv">
                <div className="notiPanel">
                    <h1>Notifications</h1>
                    <ul>
                        <li>Third Year Information Technology SEM V (2022) results declared</li>
                        <li>Third Year Information Technology SEM V  (2022) results declared</li>
                        <li>Third Year Information Technology SEM V  (2022) results declared</li>
                        <li>Third Year Information Technology SEM V  (2022) results declared</li>
            ...
            <div className="stuDiv">
                <div className="stuPanel">
                    ...
                    {(credErr && !detailsErr) ? <div id='errmsg'><p>check credentials</p></div> : ''}
                    {detailsErr ? <div id='errmsg'><p>enter credentials</p></div> : ''}
                    {netErr ? <div id='errmsg'><p>{netErr}</p></div> : ''}
                    
                    <button ref={button} id="done" onClick={HandlerSubmit}>See Results</button>
                    <Link to='/Teacher/Login' id='teacherloginlink'>Login as teacher</Link>
                </div>
            </div>
